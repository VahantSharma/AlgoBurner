...
const Index = () => {
  ...
  const generateRoast = (stats: LeetCodeStats) => {
    const roasts = [
      ...
      `${Math.round((stats.totalSolved / stats.totalQuestions) * 100)}% completion rate? Even my patience has better statistics.`,
        ...
        {error && !stats && !isLoading && (
          <div className="mt-16 max-w-md mx-auto">
            <div className="bg-red-900/20 border border-red-500 rounded-lg p-4 text-center">
              <h3 className="text-red-400 font-semibold mb-2">Oops! Something went wrong</h3>
              <p className="text-red-300 text-sm">{error}</p>
              <p className="text-slate-400 text-xs mt-2">
                Double-check the username and try again, or the API might be temporarily unavailable.
              </p>
            </div>
          </div>
        )}
